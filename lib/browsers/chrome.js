...
chrome.checkIfPageIsDoneLoading = function (tab) {
	return new Promise((resolve, reject) => {
		...
		tab.Runtime.evaluate({
			...
			util.log('numRequestsInFlight is' , tab.prerender.numRequestsInFlight);
			util.log('lastRequestReceivedAt is' , tab.prerender.lastRequestReceivedAt);
			util.log('waitAfterLastRequest is' , (new Date()).getTime() - waitAfterLastRequest);
			...
			util.log('shouldWaitForPrerenderReady is', shouldWaitForPrerenderReady);
			util.log('doneLoading is', doneLoading);
			resolve(
